  ...
  const fetchSchedules = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}${ENDPOINTS.ALLAJBYID.replace("{id}", aydi)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      <div className="bg-white overflow-x-auto dark:bg-slate-600 flex gap-3 dark:border-gray-400 border-2 rounded-lg p-5 shadow-sm max-md:w-full">
        {loading ? (
          ...
          schedules.map((schedule) => (
            ...
            <div
              key={schedule.id}
              className="otrsked flex bg-opacity-20 backdrop-blur-2xl justify-between hover:translate-y-1 hover:shadow-md cursor-grab items-center bg-palet3 dark:bg-gray-700 max-w-full py-5 px-5 transition-all rounded-full dark:text-white"
            >
                ...
                <div className="ikonsked w-11 h-11 p-1 m-0 border border-white text-center rounded-full grid place-items-center bg-palet1 dark:bg-gray-500">
                  <span className="uppercase">
                    {schedule?.agenda_name
                      ...
                      ?.join("")
                      ?.substring(0, 3) + (schedule?.agenda_name?.split(" ")?.map((word) => word.charAt(0)).join("").length > 3 ? "..." : "") || ""}
                  </span>
                </div>
      ...
      {showModal && (
        <div className="modal fixed inset-0 z-50 flex items-center justify-center bg-gray-800 bg-opacity-70 backdrop-blur-sm transition-all">
          <div className="modal-content bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg relative w-full max-w-md">
            ...
            <div className="mmbnbmnvmgjgjk flex capitalize gap-1">
            ...
            <Select
              ...
              value={clients.filter((client) => client?.value === formData.with_klien)}
              onChange={(selectedOption) => {
                const selectedClient = selectedOption ? parseInt(selectedOption.value, 10) : null; // Convert to integer
