...
const Dashboard = () => {
  ...
  const allMenuItems = [
    ...
    },
    {
      ...
      icon: (
        <svg
          class="flex-shrink-0 w-5 h-5 text-palet2 transition duration-75 dark:text-gray-400 group-hover:text-palet3 dark:group-hover:text-white"
          aria-hidden="true"
          xmlns="http://www.w3.org/2000/svg"
          fill="currentColor"
          viewBox="0 0 20 20"
  ...
  useEffect(() => {
    ...
    // console.table(storedUser);

    if (storedUser) {
      try {
        const parsedUser = JSON.parse(storedUser);
        setUserRole(parsedUser); // Asumsi struktur user di localStorage memiliki properti 'role'
      } catch (error) {
        console.error("Error parsing user data from localStorage:", error);
        // Handle error parsing, mungkin redirect ke login jika data rusak
        // navigate('/login');
      }
    } else {
      // Jika data user tidak ada di localStorage, redirect ke login
      console.log("gagal encod");
    }
  }, []);

  const userMenu = userRole && userRole.menu ? JSON.parse(userRole.menu) : [];

  // Filter daftar menu berdasarkan menu pengguna
  const filteredMenuItems = allMenuItems.filter((item) =>
    userMenu.includes(item.name)
  );
  return (
    <div className="bg-bag-primary dark:bg-bg-negmod min-h-dvh">
      ...
      >
        <div className="h-full px-3 pb-4 overflow-y-auto bg-bag-primary dark:bg-bg-negmod">
            ...
            <li>
              <Link to="/santri-all">
                <span className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-hoper dark:hover:bg-gray-700 group focus:bg-hoper focus:dark:bg-gray-700">
                  ...
                  <span className="flex-1 ms-3 whitespace-nowrap">Santri</span>
                </span>
              </Link>
            </li>
            <li>
          ...
          <ul className="space-y-2 font-medium capitalize" id="nav1"> 
            {filteredMenuItems.map((menuItem) => (
              <li key={menuItem.name}>
                <Link to={menuItem.path}>
                  <span className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-hoper  dark:hover:bg-gray-700 group active:bg-hoper focus:bg-hoper focus:dark:bg-gray-700">
                    {menuItem.icon}
                    <span className="flex-1 ms-3 whitespace-nowrap text-bag-primary-dark dark:text-white">
                      {menuItem.name}
                    </span>
