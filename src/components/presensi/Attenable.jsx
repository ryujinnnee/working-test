...
const Attenable = () => {
      ...
      if (response.status === 200) {
        ...
        if (Notification.permission === "granted") {
          new Notification("Check-in Berhasil", {
            body: "Selamat pagi!, semangat buat hari ini yaaa✨",
          });
  ...
  const getAllPresences = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`${API_URL}${ENDPOINTS.GETPRESALL}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      if (response.status === 200) {
        ...
        // toast.success('Semua presensi berhasil diambil');
      } else {
        toast.error(
          `Gagal mengambil semua presensi: ${
  ...
  const getPresencesByUserId = async (userId) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // toast.success('Presensi pengguna berhasil diambil');
      } else {
        toast.error(
  ...
  const exportDT = async () => {
    const token = getTokenFromCookie();
    try {
      const response = await axios.get(`${API_URL}${ENDPOINTS.RKPPRESME}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        responseType: "blob",
      ...
      link.href = url;
      link.setAttribute("download", "rekap-presensi.csv");
  ...
  const exportDTA = async () => {
    const token = getTokenFromCookie();
    try {
      const response = await axios.get(`${API_URL}${ENDPOINTS.RKPPRES}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        responseType: "blob",
      ...
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "rekap-presensi-all.csv");
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      toast.success("Data Berhasil Dieksport");
  ...
  return (
    <div className="flex flex-col w-full items-center min-h-dvh px-5">
      <div className="firsfxfsdfs flex justify-between w-full mt-3 max-md:flex-col">
        <div className="lefsdfsdfxxxadsf flex flex-col justify-between w-1/2 mr-3 max-md:w-full">
            ...
            {open && (
              ...
              >
                ...
                <p className="text-xs dark:text-white">
                  Start time - 08:00 am <br /> End time - 04:00 pm
                </p>
              </div>
            )}
          </div>
        ...
        <div className="dip">
          <div className="bg-white shadow-lg rounded-lg py-8 px-8 mb-8 w-fit max-md:w-full">
            ...
            >
              {" "}
              {/* Set a fixed width */}
              {time} {/* Menampilkan waktu di sini */}
            </div>
          </div>
      ...
      <div className="firsfxfsdfs flex gap-3 w-full mt-3 max-md:flex-col">
        <div className="bg-white dark:bg-slate-600 dark:border-gray-400 border-2 rounded-xl shadow-sm p-3 w-2/5 max-md:w-full">
          ...
          <div className="overflow-x-auto">
              ...
              <tbody className="divide-y divide-gray-200">
                {loading ? (
                  ...
                  <tr rowSpan="2">
                    <td
                      colSpan="4"
                      className="text-center py-4 text-xs text-gray-700 dark:text-white"
                    >
            ...
            {currentAttendance.length > 0 && (
              ...
              <button onClick={exportDT} className="rekap w-1/3 h-8 mt-3 bg-gradient-to-r text-white from-palet3 to-palet2 rounded-sm">
                Rekap
              </button>
              </div>
            )}
        ...
        <div className="bg-white dark:bg-slate-600 dark:border-gray-400 border-2 rounded-xl p-3 w-2/3 max-md:w-full">
          <h2 className="text-xl font-bold mb-2 dark:text-white">
            Presensi Anggota
          </h2>
          <div className="overflow-x-auto rounded-lg py-3">
            <table className="min-w-full table-auto border-collapse bg-white dark:bg-transparent rounded-sm border border-gray-300">
              <thead className="bg-gray-100 dark:bg-gray-400">
                <tr>
                  <th className="px-3 py-3 text-left text-xs font-bold text-gray-600 dark:text-white uppercase tracking-wider">
                    ID
                  </th>
                  <th className="px-3 py-3 text-left text-xs font-bold text-gray-600 dark:text-white uppercase tracking-wider">
                    User
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-bold text-gray-600 dark:text-white uppercase tracking-wider">
                    Status
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-bold text-gray-600 dark:text-white uppercase tracking-wider">
                    Masuk
                  ...
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-600 dark:text-white uppercase tracking-wider">
                    modified
                  </th>
                </tr>
              </thead>
                ...
                {loading ? (
                  ...
                  allAttendance.map((attendance, index) => (
                    <tr
                      ...
                      className="hover:bg-gray-300 transition-colors duration-200"
                    >
                      <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-white">
                      {index + 1}
                      </td>
                      <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-white">
                        {attendance.user_id}
                  ...
                  <tr>
                    <td
                      colSpan="6"
                      className="text-center py-4 text-sm text-gray-700 dark:text-white"
                    >
            ...
            {allAttendance.length > 0 && (
              <div className="w-full flex items-end justify-end">
              <button onClick={exportDTA} className="rekap w-1/3 h-8 mt-3 bg-gradient-to-r text-white from-palet3 to-palet2 rounded-sm">
                Rekap All
              </button>
              </div>
            )}
