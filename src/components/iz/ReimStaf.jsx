  ...
  useEffect(() => {
    const fetchMe = async () => {
      try {
        ...
        setGetId(userResponse.data.id);
        // Panggil fetchIzinAll setelah getId diset
        //   await fetchIzinAll(userResponse.data.id);
        console.log("yang di get", userResponse.data.id);
      } catch (error) {
  ...
  const fetchIzinAll = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}${ENDPOINTS.REIBYID.replace("{id}", aydi)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const data = response.data.reimbur;
      setreimbData(data);
      //   console.table(data)
      console.log("data id yang di get:", data);
      setLoading(false);
    } catch (error) {
              ...
              {loading ? (
                ...
                reimbData.map((izin, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 dark:text-white">{izin.jenis}</td>
