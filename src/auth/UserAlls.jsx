...
const UserAlls = () => {
  ...
  const [formData, setFormData] = useState({ id: "", name: "", email: "", user_type_id: "" });
  ...
  const fetchRoles = async () => {
    const akunCached = localStorage.getItem("akunCached");
  
    if (akunCached) {
      const { data, timestamp } = JSON.parse(akunCached);
    ...
    try {
      ...
      const rolesData = response.data.users;
  ...
  const handleCreate = async () => {
    try {
      ...
      localStorage.removeItem("akunCached");
  ...
  return (
    <div className="container w-full mx-auto px-4">
      ...
      <h2 className="text-3xl font-semibold m-0 p-0 dark:text-white">All Akun</h2>
      <button
          onClick={() => {
            setShowModal(true);
          }}
        ...
        <tbody className="divide-y divide-gray-200">
          {loading ? (
            ...
            roles.map((role) => (
              ...
              >
                <td className="px-4 py-3 w-11 text-sm text-gray-700 dark:text-white">{role.id}</td>
                ...
                <td className="px-4 py-4 text-sm text-gray-700 dark:text-white">
                  <button 
                    ...
                    className={role.id >= 1 && role.id <= 3 ? "cursor-not-allowed text-red-500 hover:text-red-700 dark:text-red-300 dark:hover:text-red-500" : "text-red-500 hover:text-red-700 dark:text-red-300 dark:hover:text-red-500"}
                  >
                    Delete
                  </button>
                </td>
